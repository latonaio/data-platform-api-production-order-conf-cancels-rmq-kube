...
func (c *DPFMAPICaller) headerCancel(
	...
	header.IsCancelled = input.ProductionOrderConfirmation.IsCancelled
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "ProductionOrderConfirmationHeader", "runtime_session_id": sessionID})
	if err != nil {
